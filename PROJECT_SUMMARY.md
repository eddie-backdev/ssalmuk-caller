...
## 6. 주요 변경 및 개선 사항 (v1.2)

### ① "받기 일시정지" 기능 안정성 강화
- **문제점**: "받기 일시정지"가 활성화된 상태에서 간헐적으로 자동 응답이 실행되는 문제.
- **해결**: `AutoClickService`의 `performAutoAnswer` 함수 내에 일시정지 조건을 다시 한번 확인하는 로직을 추가했습니다. 이를 통해 응답 시도 직전에 일시정지 상태를 최종적으로 확인함으로써, 지연 시간(delay) 중에 발생할 수 있는 상태 불일치 문제를 해결하고 기능의 안정성을 높였습니다.

### ② "자동 끊기" 조기 종료 버그 수정
- **문제점**: 통화 연결 후, 설정된 시간이 되기 전에 약 25~30초 만에 통화가 조기 종료되는 문제.
- **원인**: 전화 발신 시 "응답 없음"에 대비한 자동 끊기 타이머가, 실제 통화가 연결된 후에도 취소되지 않고 계속 실행되어 발생했습니다.
- **해결**: 통화가 연결되는 시점(`connectionMonitorRunnable`)에서 "응답 없음" 타이머(`noAnswerHangupRunnable`)를 명확하게 취소하도록 로직을 수정하여, 통화가 연결되면 '긴 통화 후 끊기' 로직만 실행되도록 수정했습니다.

### ③ "자동 끊기" 시간 랜덤화
- **요구사항**: 통화 종료 시간이 항상 분 단위로 정확하게 끊어지는 것을 방지하고, 더 자연스러운 패턴을 원함.
- **개선**: `scheduleLongHangup` 함수에서 분 단위로만 설정되던 통화 종료 시간을 초 단위까지 포함하여 랜덤하게 설정되도록 변경했습니다. 예를 들어, 5분~10분으로 설정 시 5분 13초, 8분 42초 등과 같이 더 다양한 시간으로 통화가 종료됩니다.

### ④ "끊기 일시정지" 기능의 오해 소지 있는 Toast 메시지 수정
- **문제점**: "끊기 일시정지"가 활성화되어 있을 때도 "N분 후 통화가 종료된다"는 안내 메시지(Toast)가 표시되어, 기능이 작동하지 않는 것처럼 보이는 문제.
- **해결**: `scheduleLongHangup` 함수의 로직을 수정하여, 실제로 통화 종료가 예약될 때만 안내 메시지가 표시되도록 변경했습니다. 일시정지 상태에서는 메시지가 나타나지 않습니다.
